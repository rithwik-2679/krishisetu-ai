...
const newRadar = `<div className="absolute top-0 right-0 bg-green-500 text-white text-xs font-bold px-3 py-1 rounded-bl-lg">
...
content = content.replace(/<div className="absolute top-0 right-0 bg-green-500 text-white text-xs font-bold px-3 py-1 rounded-bl-lg">\s*EXPLAINABLE AI DECISION ENGINE\s*<\/div>[\s\S]*?<div className="text-sm text-gray-500">No sufficient data.<\/div>\s*\)\}\s*<\/div>/, newRadar);
