...
let content = `// Official India Meteorological Department (IMD) API Integration (Demo Fallback Mode)
...
export const getImdWeatherForecast = async (district: string, state: string): Promise<WeatherForecast> => {
  ...
  const createDay = (offset: number, baseTemp: number, risk: 'Low'|'Moderate'|'High', prepBase: number) => {
    const d = new Date();
...
`;
fs.writeFileSync('src/utils/weather-service.ts', content, 'utf8');
